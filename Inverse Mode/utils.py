...
#        self.max_y = 10
    ...
    def generate_data(self, file_name, dim, Vsav_bool):

        data = scipy.io.loadmat(file_name)
        if dim == 1:
            t, x, usav, w = data["t_array"], data["x_array"], data["usav"], data["Wsav"]
            X, T = np.meshgrid(x, t)
        elif dim == 2:
    ...
    def params_to_inverse(self,args_param):
        ...
        if 'taud' in args_param:
            #MATLAB values: taud=0.125 , taur=70 , tauo=32.5 , d=0.1
        ...
        if 'taur' in args_param:
            ...
            self.taur = tf.math.exp(tf.Variable(4.204692619))  #initialised to 67
            params.append(self.taur)
        if 'tauo' in args_param:
            #self.tauo = tf.math.exp(tf.Variable(1.602))
            print("Estimatig tauo")
            self.tauo = tf.math.exp(tf.Variable(3.433987204))  #initialised to 31
    ...
    def pde_1D(self, x, y):
        ...
        tauu = tf.cast(tf.math.less_equal(self.uu, u),tf.float32)*self.taur + tf.cast(tf.math.less_equal(u, self.uu),tf.float32)*self.tauo
        Jso = Uu/tauu
        ...
        Iion = -(Jfi + Jsi + Jso + Istim) #modified
